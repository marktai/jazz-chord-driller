...
if [ ! -f src/jazz_chords/keys.py ]; then
  echo "SECRET_KEY = '"$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)"'" > src/jazz_chords/keys.py
  echo $(pwd)"/src/jazz_chords/keys.py generated"
fi
