...
export class Guess extends React.Component<GuessProps, GuessState> {
  ...
  rotateCard(i: number, n: number, e: any) {
    ...
    // $.makeArray($(`.jazz_chords-card.card-${i.toString()}`)).map((card: any) => {
  ...
  async copyToClipboard() {
    const text = `${this.state.game?.suggested_num_cards} card jazz_chords game by ${this.state.game?.author}\n${this.historyText().map((l) => l.join('')).join('\n')}\nPlay this puzzle at http://jazz_chords.marktai.com/games/${this.props.id}/guess`;
  ...
  renderCard(i: number, duplicated: boolean = false, buttonsInside: boolean = false, additionalRotation: number = 0) {
    ...
    let cardClasses = ['jazz_chords-card', `card-${i}`,];
  ...
  renderGame() {
    return (
      <Col xs={12} lg={9}>
        ...
        <Row>
          { this.state.game?.suggested_num_cards } card jazz_chords game by { this.state.game?.author }
          { this.renderHistory() }
